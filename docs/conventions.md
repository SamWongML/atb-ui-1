...
> Single source of truth for **code naming, the i18n translation glossary, and the Chinese voice guide** for the auto-tobe frontend workspace.
...
If you write auto-tobe code, change a translation, or write Chinese product copy, this is the page to reference.
...
| `packages/ui` | nothing | `@atb/core`, business logic |
...
auto-tobe's product nouns split into two categories:

- **Entity** — has a URL, a database row, an API type. In Chinese text, render as **lowercase English** so it visually reads like a type name and signals "this is a auto-tobe system entity".
...
This rule is aligned with the Chinese locale files under `packages/views/locales/zh-Hans/` — the docs are the de facto Chinese voice standard and have been battle-tested across 20+ pages.

### Entities — mixed rule (`issue` / `skill` / `task`)

`issue` / `skill` / `task` are auto-tobe's core entities. They have schema columns, API fields, and product UI labels that are all English. In Chinese text, they follow a **mixed rule** — what to use depends on where the word appears:
...
- **`skill`**: auto-tobe-specific concept with no established Chinese term.
...
| Category | Terms |
| --- | --- |
| Brands | **auto-tobe**, GitHub, Slack, Google, Anthropic, OpenAI, Claude, Codex, Cursor, Linear, Jira |
...
1. the Chinese locale files under `packages/views/locales/zh-Hans/` — the de facto Chinese voice standard, 20+ pages of consistent translation
