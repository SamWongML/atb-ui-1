...
import type { Issue, TimelineEntry } from "@atb/core/types";
...
vi.mock("@atb/ui/hooks/use-mobile", () => ({
  useIsMobile: () => mockViewport.isMobile,
}));

// useWorkspaceId() derives from useCurrentWorkspace (relative import inside
// @atb/core/hooks.tsx). vi.mock("@atb/core/paths") only intercepts
...
// Mock @atb/core/workspace/hooks
vi.mock("@atb/core/workspace/hooks", () => ({
  useActorName: () => ({
...
// Mock @atb/core/paths — after the URL-driven workspace refactor,
...
vi.mock("@atb/core/paths", async () => {
  const actual = await vi.importActual<typeof import("@atb/core/paths")>(
    "@atb/core/paths",
  );
  return {
    ...actual,
...
vi.mock("../../navigation", () => ({
  ...
  useNavigation: () => ({
    ...
    getShareableUrl: (p: string) => `https://app.auto-tobe.com${p}`,
  }),
...
// Mock issue config
vi.mock("@atb/core/issues/config", () => ({
...
vi.mock("@atb/core/issues/stores", () => ({
...
// Mock react-resizable-panels (used by @atb/ui/components/ui/resizable)
