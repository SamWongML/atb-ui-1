 ...
 *
 * Single source of truth for this workspace. (It was previously generated
 * from the Go backend's `reserved_slugs.json`; that source and its generator
 * were removed when the frontend was extracted. The destination `api` owns the
 * server-side reserved-slug list — keep the two in sync during reconciliation.)
 *
